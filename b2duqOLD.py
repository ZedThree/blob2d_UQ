...
# Define parameters & which are uncertain
vary = {
...
output_values = ["blobVel"]

# Create encoder, decoder & executor
encoder = uq.encoders.GenericEncoder(
    template_fname='b2dInp.template',
    delimiter='$',
    target_filename='b2dInp.json')
execute = ExecuteLocal('{}/runb2d.py b2dInp.json'.format(os.getcwd()))
decoder = uq.decoders.SimpleCSV(
        target_filename=output_filename,
        output_columns=output_values)
